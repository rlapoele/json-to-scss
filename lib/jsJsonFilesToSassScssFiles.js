        ...
        if(!_errorFlag) {
          ...
          if (_jsObject._jsonToScss) {
            ...
            if (_options.filename && isString(_options.filename) && _options.filename.length) {
              ...
              if (_filename.length) {
                validatedDestinationFilepaths[fileindex] = 
                  `${path.join(path.dirname(validatedDestinationFilepaths[fileindex]), _filename)}${path.extname(validatedDestinationFilepaths[fileindex])}`
              }
            }
          ...
          // variable name.
          if ('' === _options.prefix) {
            _options.prefix = `$${pathBasename(validatedDestinationFilepaths[fileindex])}: `;
          }
          ...
          if (0 === Object.keys(_jsObject).length) {
            ...
            try {
              ...
              // it.
              createPathDirectories(validatedDestinationFilepaths[fileindex]);
            
              // let's write the file.
              fs.writeFileSync(validatedDestinationFilepaths[fileindex], `${_options.prefix}${_sassString}${_options.suffix}\n`);
            
              terminal(`${emoji.get('hourglass')} ${chalk.blue(filepath)}: content converted. ${chalk.green('File created!')}\n`);
              terminal(`   ${emoji.get('+1')} ${chalk.green(validatedDestinationFilepaths[fileindex])}\n`);
            }
            catch (error) {
