  ...
  function _convertFile(filepath, fileindex) {
    ...
    if (-1 === ['.js', '.json'].indexOf(_filepathExtension)) {
      ...
      if (!_errorFlag) {
        ...
        // if the --kv (keys as vars) option was set,
        if (_keysAsVars) {
          _prefix = '$';
        }
        ...
        else if (_keysAsVars && !!Object.keys(_jsObject).length) {
          // if keysAsVars
          // loop object and make a series of keys with values
          let count = 0;
          for (const [key, value] of Object.entries(_jsObject)) {
            let _newSassString = `${jsValueToSassString(
              value,
              _indentationText,
              _indentationSize,
            ...
            _sassString = 0 === count
              ? `${_prefix}${key}: ${_newSassString}${_suffix}`
              : `${_sassString}\n${_prefix}${key}: ${_newSassString}${_suffix}`
          ...
          if (!mergeSourceFiles) {
            try {
              ...
              fs.writeFileSync(
                ...
                _flattenKeys || _keysAsVars ? `${_sassString}\n` : `${_prefix}${_sassString}${_suffix}\n`
              );
            ...
            if (mergeSassObjects && !_flattenKeys) {
              ...
              // one.
              mergedSassStrings = _flattenKeys || _keysAsVars
